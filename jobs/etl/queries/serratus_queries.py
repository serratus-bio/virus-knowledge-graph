...
def get_sra_df():
    ...
    return get_query_results(query=query, cache_filename="sql_sra_nodes.csv")


def get_palmprint_df():
    query = "SELECT * FROM public.palmdb2"
    return get_query_results(query=query, cache_filename="sql_palmprint_nodes.csv")


def get_palmprint_msa_df():
    query = (
        "SELECT * FROM public.palm_graph " "WHERE pident >= 40 AND palm_id1 != palm_id2"
    )
    return get_query_results(query=query, cache_filename="sql_palmprint_edges.csv")


def get_sra_palmprint_df():
    query = (
        "SELECT srarun.run as run_id, palm_id, percent_identity "
        "FROM palm_sra2 "
        "INNER JOIN srarun ON palm_sra2.run_id = srarun.run"
    )
    return get_query_results(query=query, cache_filename="sql_sra_palmprint_edges.csv")


def get_palmprint_sotu_df():
    query = "SELECT palm_id, sotu, percent_identity " "FROM public.palmdb2"
    return get_query_results(query=query, cache_filename="sql_palmprint_sotu_edges.csv")


def get_taxon_df():
    query = (
        "SELECT t1.tax_id, parent_tax_id, rank, genetic_code_id, "
    ...
    return get_query_results(query=query, cache_filename="sql_taxon_nodes.csv")


def get_sra_has_host_metadata_df():
    query = "SELECT srarun.run as run_id, tax_id FROM srarun"
    return get_query_results(
        query=query, cache_filename="sql_sra_host_metadata_edges.csv"
    )


def get_sra_has_host_stat_df():
    query = (
        "SELECT DISTINCT stat.run as run_id, stat.taxid as tax_id, "
    ...
    return get_query_results(query=query, cache_filename="sql_sra_host_stat_edges.csv")


def get_palmprint_taxon_edges_df():
    query = "SELECT * FROM public.palm_tax"
    return get_query_results(
        query=query, cache_filename="sql_palmprint_taxon_edges.csv"
    )


def get_sra_tissue_df():
    query = (
        "SELECT bt.biosample_id, bt.bto_id, bt.source, bt.text "
        "FROM public.biosample_tissue as bt "
        "INNER JOIN public.srarun as sr "
        "ON bt.biosample_id = sr.bio_sample "
    )
    return get_query_results(
        query=query, cache_filename="sql_biosample_tissue_edges.csv"
    )
