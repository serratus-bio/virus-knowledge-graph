...
def run(args):
    if args.task == 'all' or args.task == 'palmprint':
        ...
        feature_queries.write_to_feature_store(palmprint_fts, 'n4j_palmprint_nodes.csv')

        has_sotu_edges = graph_queries.get_has_sotu_edges()
        has_sotu_edges_fts = feature_queries.encode_edges(has_sotu_edges)
        feature_queries.write_to_feature_store(has_sotu_edges_fts, 'n4j_has_sotu_edges.csv')
    ...
    if args.task == 'all' or args.task == 'taxon':
        ...
        feature_queries.write_to_feature_store(taxon_fts, 'n4j_taxon_nodes.csv')

        has_parent_edges = graph_queries.get_has_parent_edges()
        has_parent_edges_fts = feature_queries.encode_edges(has_parent_edges)
        feature_queries.write_to_feature_store(has_parent_edges_fts, 'n4j_has_parent_edges.csv')
    ...
    if args.task == 'all' or args.task == 'edges':
        ...
        feature_queries.write_to_feature_store(has_host_edges_fts, 'n4j_has_host_edges.csv')
